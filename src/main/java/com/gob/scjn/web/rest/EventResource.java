...
public class EventResource {
	 ...
	 *         eventDTO, or with status 400 (Bad Request) if the event has already
	 *         an ID
	...
	public ResponseEntity<List<EventDTO>> getAllEvents(Pageable pageable,
		...
		HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(page,
				String.format("/api/events?eagerload=%b", eagerload));
	 ...
	 * @return the ResponseEntity with status 200 (OK) and with body the eventDTO,
	...
	@Timed
	public ResponseEntity<EventDTO> editEvent(@PathVariable Long id, @RequestBody EventDTO eventDTO) {
		return new ResponseEntity<EventDTO>(eventService.editEvent(id, eventDTO), HttpStatus.OK);
	}
}
